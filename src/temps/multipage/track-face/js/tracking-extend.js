...
(function(window, undefined) {
   ...
   */
  TrackingFace.prototype.catchView = function(
    morePeopleWarning,
    nonePeopleWarning
  ) {
    ...
    this.trackTask = tracking.track("#" + this.videoId, this.tracker, {
      camera: true
    });
    this.bindEvent();
  };
  ...
  TrackingFace.prototype.bindEvent = function() {
    ...
    _this.tracker.on("track", function(event) {
      ...
      if (event.data.length > 1) {
        ...
        // 触发回调
        if (typeof _this.morePeopleWarning === "function")
          _this.morePeopleWarning(shotImg);
      } else if (event.data.length === 0) {
        // 没有人情况-回调
        if (typeof _this.nonePeopleWarning === "function")
          _this.nonePeopleWarning();
      } else {
        // console.log('1111')
